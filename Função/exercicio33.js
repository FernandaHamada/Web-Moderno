/* 33) ​Crie três vetores, chamados vetorInteiro, vetorString e vetorDouble. 
Cada um destes vetores deverá conter quatro valores, sendo o primeiro com valores inteiros, 
o segundo com strings e o terceiro com valores decimais. Declarados os vetores, 
utilize a função de união concat() de duas maneiras diferentes para unir os vetores, 
e mostre o resultado no console. Todos os elementos do vetor resultado deverão aparecer no console. */

const vetorInteiro = [1, 2, 3, 4]
const vetorString = ["um", "dois", "três", "quatro"]
const vetorDouble = [1.0, 2.0, 3.0, 4.0]

function concat(vetorInteiro, vetorString, vetorDouble){
    let vetoriS = ""
    let vetorDS = ""
    for (let i = 0; i < vetorInteiro.length; i++){
        vetoriS = vetoriS + vetorInteiro[i] +" = "+ vetorString[i] + ", " 
        vetorDS = vetorDS + vetorString[i] + " = " + vetorDouble[i] + ", "
    }
    return [vetoriS, vetorDS]
}

console.log(concat(vetorInteiro, vetorString, vetorDouble))